...
L'architettura della versione iOS della applicazione sviluppata segue lo stesso pattern utilizzato nella versione Android, chiamato in questo caso "\textit{Navigator}" rispetto a "\textit{Single Activity}" di Android. Tramite il costrutto \textit{NavigationView}\footnote{\url{https://developer.apple.com/documentation/swiftui/navigationview}} è possibile definire una schermata "contenitore" per le viste dei percorsi di navigazione all'interno della applicazione.

    \begin{figure}[H]
    \centering
        \includegraphics[width=0.6\textwidth]{img/tesi-2-Page-20.drawio.png}
        \caption{Architettura MaggioliEbook - versione iOS}
        \label{iosarchapp}
    \end{figure}

Per navigare effettivamente tra le viste indicate è necessario utilizzare i \textit{NavigationLink}\footnote{\url{https://developer.apple.com/documentation/swiftui/navigationlink}} i quali identificano una specifica transizione di schermata da quella attuale a quella di destinazione. Le possibili schermate navigabili sono contenute, sotto forma di \textit{NavigationLink}, nel componente "\textit{Sidebar}".\\
...
Oltre alle configurazioni necessarie per impostare l'IDE XCode e l'ambiente di sviluppo, discusse a inizio della sezione \ref{iosappsec}, è stato svolto dell'ulteriore lavoro per poter utilizzare correttamente il modulo condiviso in Swift, sia nel modulo condiviso che nel modulo iOS:

\begin{itemize}
    \item \textit{Computazione Asincrona} (Coroutines)
    \begin{itemize}
        \item Realizzazione di un metodo specifico (\textit{invokeNative}) per l'invocazione dei casi d'uso. Le coroutines di Kotlin vengono mappate da KMM in funzioni con callbacks, chiamate \textit{completion handlers} e non esiste modo di definire un \textit{CoroutineScope} in Swift. In questo modo è stato possibile definire uno scope per l'esecuzione del codice asincrono anche su target iOS.
        \item impostare \textit{kotlin.native.binary.memoryModel=experimental} nel file \textit{gradle.properties} per il modulo condiviso. Senza questa configurazione ottenevo l'errore "\textit{Uncaught Kotlin exception: kotlin.IllegalStateException: There is no event loop. Use runBlocking \{ ... \} to start one.}"\footnote{\url{https://github.com/JetBrains/kotlin/blob/master/kotlin-native/NEW_MM.md\#enable-the-new-mm}}.
    \end{itemize}
    \item \textit{Tipi di dato}
    \begin{itemize}
        \item fare oggetto per convertire bytearray in data (per mostrare le cover) + oggetto per caricarle in modo async una volta scaricata (simile a funzionamento della nuova api AsyncImage fornita da Swiftui nuovo)
        \item fare extension per la gestione delle date (millisecondsSince1970)
        \item fare una extensions al libro per dire quale è l'id usato da identifiable in modo da poter ciclare su liste di libri
    \end{itemize}
\end{itemize}
...
\begin{multicols}{3}
            ...
            \begin{figure}[H]
                \includegraphics[width=0.21\textwidth]{img/reader_settings_ios.png}
                \caption{Lettura documento digitale d'esempio e impostazioni del lettore}
                \label{readersettingsios}
            \end{figure}

            \begin{figure}[H]
                \includegraphics[width=0.21\textwidth]{img/toc_ios.png}
                \caption{Elenco dei contenuti (TOC) del documento aperto e consultabile dal menu del lettore}
                \label{tocios}
            \end{figure}
            ...
            \begin{figure}[H]
                \includegraphics[width=0.21\textwidth]{img/ricerca_testo_ios.png}
                \caption{Ricerca testuale nel contenuto del documento aperto con relativo risultato}
                \label{ricerca_testoios}
            \end{figure}
            
            \begin{figure}[H]
                \includegraphics[width=0.21\textwidth]{img/ricerca_testo2_ios.png}
                \caption{Evidenziazioni automatiche delle occorrenze del testo cercato}
                \label{ricerca_testo2ios}
            \end{figure}
